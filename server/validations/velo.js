...
module.exports.veloUpdateValidator = [
    body('title', 'Необходимо название товара').optional().isLength({min: 5}).isString(),
    body('price', 'Необходимо ввести цену в правильном формате').optional().isLength({min: 3}).isString(),
    body('imageUrl', 'Неверная ссылка на изображение').optional().isString(),
    body('specifications', 'Неверный формат характеристик').optional().isArray()
]